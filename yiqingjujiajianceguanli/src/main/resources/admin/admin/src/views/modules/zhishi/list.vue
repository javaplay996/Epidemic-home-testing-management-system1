        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('zhishi','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/yiqingjujiajianceguanli/upload/zhishiMuBan.xls"
                        >批量导入健康小知识数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('zhishi','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="zhishiUploadSuccess"
                                :on-error="zhishiUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('zhishi','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入健康小知识数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('zhishi','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "zhishi.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="zhishiVideo"
                                     header-align="center"
                                     width="200"
                                     label="知识视频">
                        <template slot-scope="scope">
                            <div v-if="scope.row.zhishiVideo">
                                <video :src="$base.url+scope.row.zhishiVideo" width="100" height="100" controls="controls">
                                </video>
                            </div>
                            <div v-else>无视频</div>
                        </template>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('zhishi','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('zhishi','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('zhishi','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('zhishi','修改')" type="primary" icon="el-icon-tickets" size="mini" @click="shangxia(scope.row.id,scope.row.shangxiaTypes)">{{scope.row.shangxiaTypes == 1?'下架':'上架'}}</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'zhishi',//当前表表名,
                        sumColum : 'zhishi_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'zhishi_name',//分组字符串字段
                        // types : 'zhishi_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['zhishiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            zhishiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "zhishi/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
