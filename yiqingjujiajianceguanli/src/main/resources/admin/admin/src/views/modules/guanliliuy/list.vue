        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('guanliliuy','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/yiqingjujiajianceguanli/upload/guanliliuyMuBan.xls"
                        >批量导入每日体温数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('guanliliuy','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="guanliliuyUploadSuccess"
                                :on-error="guanliliuyUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('guanliliuy','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入每日体温数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('guanliliuy','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "guanliliuy.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="guanliliuyTiwen"
                                   header-align="center"
                                   label="体温">
                        <template slot-scope="scope">
                            {{scope.row.guanliliuyTiwen}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="guanliliuyPhoto"
                                     header-align="center"
                                     width="200"
                                     label="图片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.guanliliuyPhoto">
                                <img :src="$base.url+scope.row.guanliliuyPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="有无外出">
                        <template slot-scope="scope">
                            {{scope.row.wuaichuValue}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="guanliliuyDidian"
                                   header-align="center"
                                   label="外出地点">
                        <template slot-scope="scope">
                            {{scope.row.guanliliuyDidian}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="binghuanTypes"
                                      ...
                                      label="是否接触病患">
                        <template slot-scope="scope">
                            {{scope.row.binghuanValue}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="gaofengxianTypes"
                                      header-align="center"
                                      label="是否出入高风险区域">
                        <template slot-scope="scope">
                            {{scope.row.gaofengxianValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('guanliliuy','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('guanliliuy','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('guanliliuy','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                id : null,
                yonghuId : null,
                guanliliuyTiwen : null,
                guanliliuyPhoto : null,
                keshouTypes : null,
                wuaichuTypes : null,
                guanliliuyDidian : null,
                binghuanTypes : null,
                gaofengxianTypes : null,
                guanliliuyContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                ...
                //本表字段
                     ...
                     '有无咳嗽': "keshouValue",
                     '有无外出': "wuaichuValue",
                     '外出地点': "guanliliuyDidian",
                     '是否接触病患': "binghuanValue",
                     '是否出入高风险区域': "gaofengxianValue",
                     '添加时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'guanliliuy',//当前表表名,
                        sumColum : 'guanliliuy_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'guanliliuy_name',//分组字符串字段
                        // types : 'guanliliuy_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['guanliliuyDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            guanliliuyUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "guanliliuy/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
